...
def load_and_filter_data(
    elder_scrolls_path: str, existing_train_path: str, target_sizes: Dict[str, int]
) -> Dict[str, List[Dict]]:
    """
    ...
    skyrim_filtered = [
        r for r in es_records.get("skyrim.txt", []) if is_high_quality_elder_scrolls(r)
    ]
    oblivion_filtered = [
        r
        for r in es_records.get("oblivion.txt", [])
        if is_high_quality_elder_scrolls(r)
    ]
    morrowind_filtered = [
        r
        for r in es_records.get("morrowwind.txt", [])
        if is_high_quality_elder_scrolls(r)
    ]
    ...
    print(
        f"  ES.csv: {len(es_csv_filtered)}/{len(es_csv_records)} passed quality filters"
    )
...
def validate_splits(splits: Dict[str, List[Dict]]):
    ...
    for split_name, records in splits.items():
        ...
        print(
            f"  Persona coverage: {with_persona:,}/{len(records):,} ({persona_rate:.1f}%)"
        )
...
def main():
    ...
    filtered_data = load_and_filter_data(
        ELDER_SCROLLS_PATH, EXISTING_TRAIN_PATH, target_sizes
    )

    # Create splits
    splits = create_splits(
        filtered_data, train_ratio=0.90, val_ratio=0.05, test_ratio=0.05
    )
